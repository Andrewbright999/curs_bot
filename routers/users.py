...
async def webinar_check_in(callback: CallbackQuery, state: FSMContext):
    ...
    if answer == "Хочу участвовать✅":
        ...
        if username == None:
            await callback.message.answer(text="""<b>Благодарю за доверие❤️</b>
...
<i>Толгонай</i>
    """)
            ...
            await callback.message.answer("""<b>Благодарю за доверие❤️</b>
...
<i>Толгонай</i>""")
    ...
    if username == None:    
        await callback.message.edit_text(text="""<b>Приятно познакомиться!</b>
